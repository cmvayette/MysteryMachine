...
using SystemCartographer.Scanner.CSharp;
using SystemCartographer.Scanner.Sql;

namespace SystemCartographer.Cli;
...
public class Program
{
    ...
    {
        Console.WriteLine("""
            System Cartographer - Atomic-level codebase intelligence

            Usage: cartographer <command> [options]
            ...
            Update Options:
              --snapshot <file>   Snapshot file to upload (required)
              --connection <str>  Database connection string (required)
    ...
    {
        Console.WriteLine("System Cartographer v0.2.0");
        return 0;
    }
    ...
    private static async Task<int> ExecuteUpdate(string[] args)
    {
        string? snapshotPath = null;
        ...
        if (string.IsNullOrEmpty(snapshotPath) || string.IsNullOrEmpty(connection))
        {
            Console.WriteLine("Error: --snapshot and --connection are required");
            return 1;
        }

        Console.WriteLine($"📤 Uploading snapshot to database...");
        Console.WriteLine($"   Snapshot: {snapshotPath}");

        // TODO: Implement actual database upload using Microsoft.Data.SqlClient
        await Task.Delay(100); // Placeholder

        Console.WriteLine("✅ Global map updated successfully.");
        return 0;
    }
